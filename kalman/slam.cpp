...
void slam_insert_v(float vx/*cm/s*/, float vy/*cm/s*/, unsigned short heading, unsigned int tv_msec) {
	...
	slam_xr += ((vy*sin(RAD(tmp_heading)) + vx*cos(RAD(tmp_heading)))*tmp_msec)/SLAM_RESOLUTION; /*Zentimeter*/
	slam_yr += ((vx*sin(RAD(tmp_heading)) + vy*cos(RAD(tmp_heading)))*tmp_msec)/SLAM_RESOLUTION; /*Zentimeter*/
...
void slam_refresh_pos(unsigned short tv_msec) {
	...
	if (!(env_no.size() == 0 && env_nw.size() == 0 && env_sw.size() == 0 && env_so.size() == 0)) {
		...
		int Nxd4 = Nxd/4; int Nxd2 = Nxd/2;
		int Nyd4 = Nyd/4; int Nyd2 = Nyd/2;
		fftwf_complex *A, *C, *D;
		fftwf_plan p1, p2, p3;
		...
		for (int i = 0; i < Nxd; i++) {
			...
			if ((i > Nxd2)) {
				...
				for (int j = Nyd2; j < Nyd; j++) {
					A[iNyd +j][0] = 0;
					A[iNyd +j][1] = 0;
				}
				for (int j = 0; j < Nyd2; j++) {
					unsigned char tmp = slam_get_tmp(i-(Nxd4),j-(Nyd4));
		...
		for (int i = 0; i < Nxd; i++) {
			...
			if ((i > Nxd2)) {
				...
				for (int j = 0; j < Nyd2; j++) {
					unsigned char tmp = slam_get(i+approx_pos_acc.x-(Nxd4),j+approx_pos_acc.y-(Nyd4));
					if (tmp == ENV_ITEM_PATH) tmp = 0;
					else if (tmp == ENV_ITEM_APPROX) tmp = 1;
					else if (tmp == ENV_ITEM_SOLID) tmp = 1;
					else if (tmp == ENV_ITEM_UNSOLID) tmp = 0;
		...
		for (int i = 0; i < Nxd; i++) {
				for (int j = 0; j < Nyd; j++) {
					int iNyd = i*Nyd;
					float tmp = C[iNyd + j][0]*D[iNyd + j][0] + C[iNyd + j][1]*D[iNyd+ j][1];
					C[iNyd+j][1] = C[iNyd + j][1]*D[iNyd + j][0] - C[iNyd + j][0]*D[iNyd + j][1];
					C[iNyd+j][0] = tmp;
				}
		}
		...
		for (int i = 0; i < Nxd; i++) {
			for (int j = 0; j < Nyd; j++) {
				if (A[i*(Nxd)+j][0] > max) {
					max = A[i*(Nyd)+j][0];
					if (i <= (Nxd2 + 1))	max_pos.x = i;
					else 			max_pos.x = i - Nxd;
					if (j <= (Nyd2 + 1))	max_pos.y = j;
	...
	acc_tv_msec = tv_msec; slam_xr = 0; slam_yr = 0;
	for(unsigned int i = 0; i < env_no_tmp.size(); i++) {
